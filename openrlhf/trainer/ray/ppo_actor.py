...
# def calculate_kl_weight(x): # f(d)=ℯ^(-(e * d))
#   if not 0.0 <= x <= 1.0:
...
def calculate_kl_weight(x): # f(d) = 1 - e^(-e * (1 - d))
  if not 0.0 <= x <= 1.0:
    raise ValueError("value Error")
  if x == 0.0 or x == 1.0:
     return 1.0
  e = math.exp(1) 
  weight = 1 - math.exp(-e * (1 - x))
  return weight
...
class ActorPPOTrainer(BasePPOTrainer):
    def __init__(
        ...
        **kwargs,
        ...
        self.diff2KL_weight = calculate_kl_weight
    ...
    def training_step(self, experience: Experience) -> Dict[str, float]:
        ...
        difficulties = experience.difficulty
        neg_diff_weights = experience.neg_diff_weight

        KL_weights = torch.ones_like(difficulties, dtype=difficulties.dtype, device=difficulties.device)
        for idx, (advantage, difficulty) in enumerate(zip(advantages, difficulties)):
            if advantage[0] > 0:
                KL_weights[idx] = self.diff2KL_weight(difficulty.item())
        ...
        if self.args.use_kl_loss:
            ...
            if self.args.use_dads:
                # print(f"kl_mean: {kl_mean}")
                # print(f"KL_weights: {KL_weights}")
                kl_mean = kl_mean * KL_weights
...
class ActorModelRayActor(BasePPORole):
    ...
    def prepare_datasets(self):
        ...
        print('='*20+'NOT SHUFFLE!!!! NOTNOT SHUFFLE'*2+'='*20)
